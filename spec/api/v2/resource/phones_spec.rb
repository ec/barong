...
describe 'Api::V2::Resources::Phones' do
  ...
  context 'Twilio Whatsapp Service' do
    before do
      allow(Barong::App.config).to receive(:twilio_provider).and_return(TwilioWhatsappService)
    end
    ...
    describe 'POST /api/v2/resource/phones' do
      ...
      context 'valid story' do
        before do
          allow(Barong::App.config.twilio_provider).to receive(:send_sms).and_return(false)
        end
